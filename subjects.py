...
def subjectWindow():
    ...
    savebtn = tk.Button(window,text="Delete",command=delete)
    savebtn.grid(column=2, row = 7)
    savebtn = tk.Button(window,text="Update", command=update)
    ...
    schedFilter = Entry(window, textvariable=schd,width = 10) 
    schedFilter.grid(column=8,row=7)

    filterbtn = Button(window,text="Filter", command=setFilter)
